  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const profilePayload = {
        ...
        facultyId: formData.facultyId.trim().toUpperCase(),
      ...
      <div className="bg-white dark:bg-[#1E293B] rounded-[2.5rem] shadow-2xl w-full max-w-5xl flex flex-col md:flex-row overflow-hidden border border-white dark:border-slate-700">
        ...
        <div className="md:w-1/2 p-12 bg-white dark:bg-[#1E293B] flex flex-col justify-center">
          ...
          <h1 className="text-5xl font-black text-[#1E293B] dark:text-white leading-tight mb-4">
            Empower Your <span className="text-[#6366F1] dark:text-[#818CF8]">Classroom</span> <br /> Management.
          </h1>
        ...
        <div className="md:w-1/2 bg-[#F8FAFF] dark:bg-[#0F172A]/50 p-10 border-l border-slate-100 dark:border-slate-700">
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <RegInput label="Faculty ID" name="facultyId" value={formData.facultyId} onChange={handleChange} placeholder="FAC123" />
              <RegInput label="Full Name" name="name" value={formData.name} onChange={handleChange} placeholder="Dr. Smith" />
            </div>
            <RegInput label="Institution / College" name="college" value={formData.college} onChange={handleChange} placeholder="College Name" />
            
            <div className="grid grid-cols-2 gap-4">
              <RegInput label="Department" name="dept" value={formData.dept} onChange={handleChange} placeholder="e.g. CSE" />
              <RegInput label="Designation" name="designation" value={formData.designation} onChange={handleChange} placeholder="e.g. Asst. Professor" />
            </div>
            ...
            >
              {loading ? <Loader2 className="animate-spin" size={20} /> : "REGISTER FACULTY"}
            </button>
          </form>
          
          <p className="text-center mt-6 text-sm font-semibold text-slate-400 dark:text-slate-500">
            Already have an account? <Link href="/Teachers/login" className="text-[#6366F1] dark:text-[#818CF8] hover:underline">LOG IN</Link>
          </p>
        </div>
      </div>
    </div>
